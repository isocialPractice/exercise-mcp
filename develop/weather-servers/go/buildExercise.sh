...
_package() {
  _main helper-file GOMOD "go.mod"
}
...
_run() {
  ...
  # go.mod ships without require lines, so tidy resolves the sdk first.
  go mod tidy
  # Build next to assets/ so the draw tool finds the artwork.
  go build -o weather-server .
  echo
  echo "The MCP client starts the server, so launch it from here only to test:"
  echo "  \"$_scriptDir/weather-server/weather-server\""
}
...
_runBuildExercise() {
  ...
  if [ "$1" = "1" ]; then
    ...
    _main helper-file UTILS-MAKE-NWS-REQUEST "utils/make_nws_request.go"
    _main helper-file UTILS-FORMAT-ALERT "utils/format_alert.go"
    _main helper-file CATEGORIZE-LOCAL-WEATHER "utils/categorize_local_weather.go"
    _main helper-file GUI-RENDER-WEATHER "gui/gui_render_weather.go"
    _main helper-file GUI-DRAW-WEATHER "gui/gui_draw_weather.go"
    _main server-file SERVER "main.go"
