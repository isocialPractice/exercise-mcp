...
:_main
 if "%1"=="1" (
  if "%2"=="helper-file" (
   ...
   echo: & echo Creating 'weather.rb' File:
  ) else (
...
:_run
 if "%1"=="1" (
  ...
  echo   cd /D "%~dp0weather-server" ^&^& bundle exec ruby weather.rb
 )
goto:eof
...
:_runBuildExercise
 ...
 if "%1"=="1" (
  ...
  call :_main 1 helper-file UTILS-MAKE-NWS-REQUEST "utils\make_nws_request.rb"
  call :_main 1 helper-file UTILS-FORMAT-ALERT "utils\format_alert.rb"
  call :_main 1 helper-file CATEGORIZE-LOCAL-WEATHER "utils\categorize_local_weather.rb"
  call :_main 1 helper-file GUI-RENDER-WEATHER "gui\gui_render_weather.rb"
  call :_main 1 helper-file GUI-DRAW-WEATHER "gui\gui_draw_weather.rb"
  call :_main 1 server-file SERVER "weather.rb"
