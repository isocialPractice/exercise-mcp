...
_package() {
  _main helper-file GEMFILE "Gemfile"
}
...
_run() {
  ...
  echo "  cd \"$_scriptDir/weather-server\" && bundle exec ruby weather.rb"
}
...
_runBuildExercise() {
  ...
  if [ "$1" = "1" ]; then
    ...
    _main helper-file UTILS-MAKE-NWS-REQUEST "utils/make_nws_request.rb"
    _main helper-file UTILS-FORMAT-ALERT "utils/format_alert.rb"
    _main helper-file CATEGORIZE-LOCAL-WEATHER "utils/categorize_local_weather.rb"
    _main helper-file GUI-RENDER-WEATHER "gui/gui_render_weather.rb"
    _main helper-file GUI-DRAW-WEATHER "gui/gui_draw_weather.rb"
    _main server-file SERVER "weather.rb"
