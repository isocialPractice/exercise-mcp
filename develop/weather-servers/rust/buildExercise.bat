...
:_package
 if "%1"=="1" (
  call :_main 1 helper-file CARGO "Cargo.toml"
 )
goto:eof
...
:_main
 if "%1"=="1" (
  if "%2"=="helper-file" (
   ...
   echo: & echo Creating 'src/main.rs' File:
  ) else (
...
:_run
 if "%1"=="1" (
  ...
  cargo build --release
  rem The draw tool looks for assets\ next to the binary first.
  if NOT EXIST "target\release\assets" mkdir "target\release\assets" >nul 2>nul
  copy /Y "assets\*" "target\release\assets\" >nul
  echo:
  echo The MCP client starts the server, so launch it from here only to test:
  echo   "%~dp0weather-server\target\release\weather-server.exe"
 )
goto:eof
...
:_runBuildExercise
 ...
 if "%1"=="1" (
  ...
  call :_main 1 helper-file UTILS "src\utils.rs"
  call :_main 1 helper-file CATEGORIZE-LOCAL-WEATHER "src\categorize_local_weather.rs"
  call :_main 1 helper-file GUI-RENDER-WEATHER "src\gui_render_weather.rs"
  call :_main 1 helper-file GUI-DRAW-WEATHER "src\gui_draw_weather.rs"
  call :_main 1 server-file SERVER "src\main.rs"
