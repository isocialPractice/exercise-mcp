...
_package() {
  _main helper-file CARGO "Cargo.toml"
}
...
_main() {
  ...
  if [ "$_kind" = "helper-file" ]; then
    ...
    echo
    echo "Creating 'src/main.rs' File:"
  else
...
_run() {
  ...
  cargo build --release
  # The draw tool looks for assets/ next to the binary first.
  mkdir -p target/release/assets
  cp -f assets/*.svg target/release/assets/
  echo
  echo "The MCP client starts the server, so launch it from here only to test:"
  echo "  \"$_scriptDir/weather-server/target/release/weather-server\""
}
...
_runBuildExercise() {
  ...
  if [ "$1" = "1" ]; then
    ...
    _main helper-file UTILS "src/utils.rs"
    _main helper-file CATEGORIZE-LOCAL-WEATHER "src/categorize_local_weather.rs"
    _main helper-file GUI-RENDER-WEATHER "src/gui_render_weather.rs"
    _main helper-file GUI-DRAW-WEATHER "src/gui_draw_weather.rs"
    _main server-file SERVER "src/main.rs"
