...
:_package
 if "%1"=="1" (
  call :_main 1 helper-file CSPROJ "WeatherServer.csproj"
 )
goto:eof
...
:_run
 if "%1"=="1" (
  ...
  echo   dotnet run --project "%~dp0weather-server" --no-build
 )
goto:eof
...
:_runBuildExercise
 if "%1"=="0" (
  if NOT "%_checkParOneBuildExercise%"=="--" (
   if "%_parOneBuildExercise%"=="--build" (
    ...
    if NOT EXIST "%~dp0weather-server\Tools"  mkdir "%~dp0weather-server\Tools"  >nul 2>nul
 ...
 if "%1"=="1" (
  ...
  call :_main 1 helper-file TOOLS-WEATHER "Tools\WeatherTools.cs"
  call :_main 1 helper-file UTILS-MAKE-NWS-REQUEST "Utils\HttpClientExt.cs"
  call :_main 1 helper-file UTILS-FORMAT-ALERT "Utils\FormatAlert.cs"
  call :_main 1 helper-file CATEGORIZE-LOCAL-WEATHER "Utils\CategorizeLocalWeather.cs"
  call :_main 1 helper-file GUI-RENDER-WEATHER "Gui\GuiRenderWeather.cs"
  call :_main 1 helper-file GUI-DRAW-WEATHER "Gui\GuiDrawWeather.cs"
  call :_main 1 server-file SERVER "Program.cs"
