...
_package() {
  _main helper-file CSPROJ "WeatherServer.csproj"
}
...
_run() {
  ...
  echo "  dotnet run --project \"$_scriptDir/weather-server\" --no-build"
}
...
_runBuildExercise() {
  if [ "$1" = "0" ]; then
    ...
    if [ "$_parOneBuildExercise" = "--build" ]; then
      ...
      mkdir -p assets Tools Utils Gui
  ...
  if [ "$1" = "1" ]; then
    ...
    _main helper-file TOOLS-WEATHER "Tools/WeatherTools.cs"
    _main helper-file UTILS-MAKE-NWS-REQUEST "Utils/HttpClientExt.cs"
    _main helper-file UTILS-FORMAT-ALERT "Utils/FormatAlert.cs"
    _main helper-file CATEGORIZE-LOCAL-WEATHER "Utils/CategorizeLocalWeather.cs"
    _main helper-file GUI-RENDER-WEATHER "Gui/GuiRenderWeather.cs"
    _main helper-file GUI-DRAW-WEATHER "Gui/GuiDrawWeather.cs"
    _main server-file SERVER "Program.cs"
