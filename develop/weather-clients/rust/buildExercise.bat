...
:_run
 if "%1"=="1" (
  ...
  if NOT EXIST "..\..\..\weather-servers\rust\weather-server\target\release\weather-server.exe" (
   call ..\..\..\weather-servers\rust\buildExercise.bat --build
   cd /D "%~dp0weather-client"
  )
  call cargo run -- ..\..\..\weather-servers\rust\weather-server\target\release\weather-server.exe
 )
goto:eof
...
:_runBuildExercise
 if "%1"=="0" (
  if NOT "%_checkParOneBuildExercise%"=="--" (
   if "%_parOneBuildExercise%"=="--build" (
    ...
    echo:
    if NOT EXIST "%~dp0weather-client" cargo new "%~dp0weather-client" >nul 2>nul
    copy /Y "%~dp0README.md" "%~dp0weather-client\build_source.data.txt"
    cd /D "%~dp0weather-client"
    if NOT EXIST "%~dp0weather-client\src" mkdir "%~dp0weather-client\src" >nul 2>nul
 ...
 if "%1"=="1" (
  ...
  call :_main 1 client-file CLIENT "src\main.rs"
...
:_closeOut
 ...
 if "%_parOneBuildExercise%"=="--build" (
  ...
  call develop\weather-servers\rust\buildExercise.bat --reset
 )
