...
_serverDir="$_scriptDir/../../weather-servers/rust"
_optOut=0
...
_run() {
  ...
  if [ ! -x "$_serverDir/weather-server/target/release/weather-server" ]; then
    bash "$_serverDir/buildExercise.sh" --build
  fi
  cargo run -- "$_serverDir/weather-server/target/release/weather-server"
}
...
_runBuildExercise() {
  if [ "$1" = "0" ]; then
    ...
    if [ "$_parOneBuildExercise" = "--build" ]; then
      ...
      echo
      if [ ! -d "$_scriptDir/weather-client" ]; then
        cargo new "$_scriptDir/weather-client"
      fi
      cp -f "$_scriptDir/README.md" "$_scriptDir/weather-client/build_source.data.txt"
      mkdir -p "$_scriptDir/weather-client/src"
  ...
  if [ "$1" = "1" ]; then
    ...
    _main client-file CLIENT "src/main.rs"
