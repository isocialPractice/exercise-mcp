...
_serverDir="$_scriptDir/../../weather-servers/ruby"
_optOut=0
...
_run() {
  ...
  if [ ! -f "$_serverDir/weather-server/weather.rb" ]; then
    bash "$_serverDir/buildExercise.sh" --build
  fi
  bundle exec ruby client.rb "$_serverDir/weather-server/weather.rb"
}
...
_runBuildExercise() {
  ...
  if [ "$1" = "1" ]; then
    ...
    _main client-file CLIENT "client.rb"
