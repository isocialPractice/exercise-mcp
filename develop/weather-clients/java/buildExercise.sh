...
_serverDir="$_scriptDir/../../weather-servers/java"
_srcJava="src/main/java/org/example/weatherclient"
_optOut=0
...
_main() {
  ...
  if [ "$_kind" = "helper-file" ]; then
    ...
    echo
    echo "Creating 'McpClientApplication.java' File:"
  else
...
_run() {
  ...
  if [ ! -f "$_serverDir/weather-server/target/weather-server-0.0.1-SNAPSHOT.jar" ]; then
...
_runBuildExercise() {
  if [ "$1" = "0" ]; then
    ...
    if [ "$_parOneBuildExercise" = "--build" ]; then
      ...
      echo
      mkdir -p "$_scriptDir/weather-client/src/main/resources"
      mkdir -p "$_scriptDir/weather-client/$_srcJava"
  ...
  if [ "$1" = "1" ]; then
    ...
    _main helper-file APPLICATION-YML "src/main/resources/application.yml"
    _main helper-file SERVERS-CONFIG "src/main/resources/mcp-servers-config.json"
    _main client-file CLIENT "$_srcJava/McpClientApplication.java"
