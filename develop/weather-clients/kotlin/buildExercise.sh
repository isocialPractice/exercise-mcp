...
_serverDir="$_scriptDir/../../weather-servers/kotlin"
_optOut=0
...
_run() {
  ...
  if [ ! -f "$_serverDir/weather-server/build/libs/weather-server-0.1.0-all.jar" ]; then
    bash "$_serverDir/buildExercise.sh" --build
  fi
  java -jar build/libs/kotlin-mcp-client-0.1.0-all.jar "$_serverDir/weather-server/build/libs/weather-server-0.1.0-all.jar"
}
...
_runBuildExercise() {
  if [ "$1" = "0" ]; then
    ...
    if [ "$_parOneBuildExercise" = "--build" ]; then
      ...
      echo
      mkdir -p "$_scriptDir/weather-client/src/main/kotlin"
  ...
  if [ "$1" = "1" ]; then
    ...
    _main client-file CLIENT "src/main/kotlin/Main.kt"
