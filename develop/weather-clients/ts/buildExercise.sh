...
_serverDir="$_scriptDir/../../weather-servers/ts"
_optOut=0
...
_run() {
  ...
  if [ ! -f "$_serverDir/weather-server/build/index.js" ]; then
    bash "$_serverDir/buildExercise.sh" --build
  fi
  node build/index.js "$_serverDir/weather-server/build/index.js"
}
...
_runBuildExercise() {
  ...
  if [ "$1" = "1" ]; then
    ...
    _main client-file CLIENT "index.ts"
